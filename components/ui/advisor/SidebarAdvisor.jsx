...
export default function SidebarProject({
  ...
  /** ========= เมนูหลัก ========= */
  const menuItems = [
    { name: "ภาพรวม", icon: Home, path: "/advisor/dashboard" },
    { name: "ข่าวสาร", icon: FileText, path: "/advisor/news" },
    {
      name: "ข้อมูลนิสิต",
      icon: User,
      ...
      subItems: [
        {
          name: "เพิ่มข้อมูลนิสิต",
          icon: FilePlus2,
          path: "/advisor/student/add",
        },
      ],
    },
  ];

  /** ✅ เปิดเมนูย่อยอัตโนมัติเมื่ออยู่ในหน้า add student */
  useEffect(() => {
    if (pathname.includes("/advisor/student/add")) {
      setOpenSubmenu(true);
    }
  }, [pathname]);
    ...
    <aside className={sidebarClass}>
      ...
      <nav className="menu-list">
        {menuItems.map((item) => {
          ...
          const isNewsActive =
            item.name === "ข่าวสาร" &&
            (pathname === "/advisor/news" ||
              pathname.startsWith("/advisor/news/add") ||
              pathname.startsWith("/advisor/news/edit"));

          // ✅ Active ทั่วไป
          const isActive =
            pathname === item.path ||
            (hasSubmenu && pathname.startsWith(item.path)) ||
            isNewsActive;

          // ✅ Parent active สำหรับข้อมูลนิสิต
          const isParentActive =
            pathname.includes("/advisor/student") &&
            !pathname.includes("/advisor/student/add");
